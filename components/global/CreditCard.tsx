...
export default function CreditCard({ track, style }: Props) {
    ...
    return (
        <div className="relative text-white w-full overflow-hidden cursor-pointer shadow-md shadow-blue-800 rounded-lg">
            <div className="flex flex-col relative peer z-10 justify-between h-48 w-full p-5">
                <div className="flex font-semibold mb-2 text-13 justify-between items-center">
                ...
                <div className="">
                    <p className="text-sm font-bold"> 4000 0566 5566 5556</p>
                    <p className="text-xs">5434</p>
                </div>
                <p className={`font-medium text-xs text-end`}>01/80</p>
                <div className="flex text-11 justify-between items-center">
                    <p className="uppercase font-semibold">Tony Cordiela Johnson</p>
                    <p className="font-medium">Amex</p>
                </div>
            </div>
            <img src={`${images[track >= images.length ? track - images.length : track]}`} className="h-full w-full  peer-hover:scale-125 rounded-xl object-cover duration-500 absolute top-0 left-0 z-0" alt="" />
        </div>
    );
}
